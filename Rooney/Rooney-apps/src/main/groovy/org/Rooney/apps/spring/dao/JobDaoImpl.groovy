...
class JobDaoImpl extends BaseDao implements JobDao {
	...
	@Override
	public long countForQuery(String jobStatus, String cronExpression,int jobType) {
		def sql=new StringBuilder()
		sql.append("SELECT COUNT(*) FROM hadoop_quartz_job WHERE job_type>=${jobType}")
		if(jobStatus){
			sql.append(" AND job_status='${jobStatus}'")
		}
		if(cronExpression){
	...
	public ResultMsg insert(Jobs job) {
		...
		def sql="""
				...
				'${job.jobGroupCn}','${job.jobNameCn}',${job.jobType},'${job.jobDsc}',${job.createId},${job.createTime})
				"""
	...
	public ResultMsg update(Jobs job) {
		...
		def sql="""
				...
				job_name_cn='${job.jobNameCn}',job_type=${job.jobType},job_dsc='${job.jobDsc}',update_id=${job.updateId},update_time=${job.updateTime}
				 WHERE id=${job.id}
				"""
		def n=template.update(sql)
		if(n>0){
	...
	public Jobs findOne(long id) {
		def sql="SELECT * FROM hadoop_quartz_job WHERE id=${id}"
		RowMapper<Jobs> rm={ ResultSet rs,int rowNum->
			def job=new Jobs()
			job.num=rowNum+1
			job.id=rs.getLong(1)
