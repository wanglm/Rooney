...
(/** @lends <global> */function( $ ) {
	...
	function _fnUpdateInfo ( settings )
	{
		...
		var
			...
			max   = isAjaxInit?settings.fnRecordsTotal():initTotal,
			total = isAjaxInit?settings.fnRecordsDisplay():initTotal,
			out   = total ?
				lang.sInfo :
				lang.sInfoEmpty;
	...
	function _fnInfoMacros ( settings, str )
	{
		...
		var
			...
			vis        = isAjaxInit?settings.fnRecordsDisplay():initTotal,
			all        = len === -1;
	
		return str.
			replace(/_START_/g, formatter.call( settings, start ) ).
			replace(/_END_/g,   formatter.call( settings, isAjaxInit?settings.fnDisplayEnd():10) ).
				...
				"fn": function( settings ) {
					if ( modern ) {
						...
						var
							...
							len        = settings._iDisplayLength,
							visRecords = isAjaxInit?settings.fnRecordsDisplay():initTotal,
							all        = len === -1,
	...
	function _fnPageChange ( settings, action, redraw )
	{
		...
		var
			start     = settings._iDisplayStart,
			len       = settings._iDisplayLength,
			records   = isAjaxInit?settings.fnRecordsDisplay():initTotal;
