package org.Rooney.hadoop.mapreduce.inputformat;
...
public class LogInputFormat extends FileInputFormat<Text, Text> {
	...
	public RecordReader<Text, Text> createRecordReader(InputSplit split,
		...
		String regex = null;
		/*
		 * try { //注解的方式传值，当然要是动态改变的话，貌似只能动Configuration了。。。现在不实现 Class<?>
		 * clz=context.getInputFormatClass(); Where
		 * where=clz.getAnnotation(Where.class); if(where!=null){
		 * regex="";//想个方法弄出正则才行。。。现在想不出来... } logReader=this.new
		 * LogRecordReader(recordDelimiterBytes, regex); } catch
		 * (ClassNotFoundException e) { LOG.error("反射获取inputFormatClass出错：", e);
			...
			while (getFilePosition() <= end
				...
				if (pos == 0) {
					...
					if (StringUtils.isBlank(regex)) {
						...
						lineStr = lineText.toString();
						strs = lineStr.split(",");
						if (strs.length >= 2) {
							key.set(strs[0]);
							value.set(strs[1]);
						} else {
							LOG.info("特殊的长度<2的行：----" + lineStr);
						}
					}
					pos += newSize;
				}
